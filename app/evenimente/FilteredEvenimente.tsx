...
import SpectacolCard from "@/app/spectacole/SpectacolCard";
...
// Local-time key for calendar day buttons (generated from local Date constructor)
function toDateKey(d: Date): string {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

// UTC key for event dates stored as timestamptz midnight UTC
...
export default function FilteredEvenimente({ listings }: { listings: Listing[] }) {
  ...
  // End of this week = Sunday (JS: day 0)
  ...
  // Keep only events that haven't fully ended yet
  ...
  // Mark every day covered by an event (including multi-day ranges) on the calendar
  ...
  // Apply calendar day or temporal filter
  const filtered = useMemo(() => {
    if (selectedDay) {
      // Show events that include the selected day (event_date <= day <= event_end_date)
  ...
  return (
    <>
      ...
      <p className="text-sm font-semibold text-gray-400 mb-4">
        {filtered.length} {filtered.length === 1 ? "eveniment găsit" : "evenimente găsite"}
      </p>
